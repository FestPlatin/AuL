...
\section{Hashing}
Gegeben sei eine Menge \(U\) von Schlüsseln und eine Menge \(S \subseteq U\) von zu verwaltenden Schlüssel.
...
Die Hastabelle \(T\) ist ein Array von Zeigern welches auf die eigentlichen Datensätze zeigt.
\begin{figure}[htbp]
	\begin{center}
...
Wenn wir annehmen, dass \(h\) injektiv ist, lässt sich die Hashfunktionen sehr einfach implentieren:
\begin{description}
	\item[Suche] if \(T(h(s)) \neq NIL \dots\)
...
Da \(h\) jedoch nicht injektiv ist, tretten Kollisionen auf.
...
Die Hashfunktion sollte daher gut streuen.
Eine gute Funktion ist \(h(s)=s \mod m\) wobei \(m\) eine Primzahl sein sollte.
Die einfache Art der Kollisionsbehandlung ist Hashing mit Verkettung.
...
Wenn \(\beta\) beschränkt ist, bleibt die Suchzeit dabei in \(\mathcal{O}(1)\).

Beim Überschreiten einen Schwellwertes für \(\beta\) (typisch \(\beta = \frac{3}{4}\)) werden alle Einträge in eine größere Hashtabelle kopiert.
Entsprechend beim Unterschreiten eine Schwellwertes in eine kleinere Hashtabelle.
Die amortisierte Laufzeit (d.h. wir verteilen den Aufwand zum Kopieren auf alle vorherigen Hashoperationen) beträgt ebenfalls \(\mathcal{O}(1)\).
...
Beim Zusammenfügen fällt auf jeder Ebene der Aufwand \(\mathcal{O}(n)\) an.
...
\begin{eqnarray*}
	...
	V(n+1) &=& 2n + 4V(\frac{2}{4}) \\
		&\vdots&	\\
		&=& k \cdot n + 2^{k} \cdot V(\frac{2}{2^{k}})	\\
		&=&n \cdot \log n
\end{eqnarray*}
